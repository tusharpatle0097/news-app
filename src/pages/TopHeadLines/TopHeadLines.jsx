...
import { topHeadLines } from '../../apis/Api';
// #151a1b
...
const TopHeadLines = () => {
    ...
    return (
        <>
            <div className='text-center text-[30px] font-bold my-5'>Today Top Headlines</div>
                ...
                {data.length > 0 ? (data.map((items, index) => {
                    return (
                        <div key={index} className={` ${DarkModeEB ? 'bg-[#151a1b]' : ''} max-w-xs rounded overflow-hidden shadow-lg h-[35rem]`}>
                            <img className="w-full h-[13rem]" src={items.urlToImage} alt={items.title} />
